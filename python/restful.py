...
class Test(Restful):#测试1
    ...
    def do_GET(self): #重写get方法给了通过客户端请求的url找到对应资源
        self.done()
        print "INFO: path="+self.path
        print "INFO: dp="+self.dp
        print [ r for r in self.router.keys()]
...
try:
    #server=HTTPServer(('',8084),Test) #测试1
